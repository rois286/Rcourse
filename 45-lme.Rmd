...
Supposedly, more observations imply more information on the mean. This, however, is not the case if samples are completely dependant. More observations do not add any new information.
From this example one may think that dependence is a bad thing. This is a false intuition: negative correlations imply oscilations about the mean, so they are actually more informative on the mean than independent observations. 
```
...
Also assume that some subjects received some treatment, and others did not. 
...
For a rough intuition, think of a case where observations within subject are perfectly dependent. 
```
...
This implies the within-machine variability is the only source of variability we care about, because the subtraction of the machine effect, removed information on the between-machine variability.  
...
- __MANOVA__:
Many of the problems that may be solved with a multivariate analysis of variance (MANOVA), may be solved with an LMM for reasons we detail in chapter \@ref(multivariate).
...
Put differently, if we ignore the statistical dependence in the data we will probably be making more errors than possible/optimal.
...
1. If you are using the model merely for predictions, and not for inference on the fixed effects or variance components, then stating the generative distribution may be useful, but not necessarily. 
...
What if the correlation does not have a block structure? 
